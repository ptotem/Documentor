...
ComponentSpec.destroy_all
ComponentType.destroy_all
...
@superset_count=SuperSet.all.count
  @sup_set=SuperSet.create!(:name => "SuperSet#{@superset_count+1}")
  ...
  sheet1.each 1 do |row|
    ...
    if !@component.nil?
      ...
      5.times do |i|
        @ref_set=ReferenceSet.find_by_name_and_super_set_id("ref_set#{i+1}",@sup_set.id)
        if @ref_set.nil?
        ...
        if row[1].downcase=="text"
          ...
          if !row[i+3].blank?
            ...
            @example.example_image=File.open("#{args[:src_dir_path]}/images/blank.png")
          end
